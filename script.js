...
tHead.addEventListener("keyup", (event) => {
  ...
  const { 1: columnNumber } = event.target.id.split("-"); // id = "0-3" => after .split => [0, 3]  // 1 index ki value phar column number rakh diya jayga
...
function getColumnSearchInputElement(columnNumber) {
  ...
  columnSearchInputElement.setAttribute("Placeholder", "Search here");
...
function createTableUIFromLocalStorage() {
  ...
  tableState.header.forEach((element) => {
   ...
   //   header swap
    ...
    columnElement.ondrop = function (e) {
    const droppedElemIdx =  e.dataTransfer.getData('text/plain').split('-')[1]
    const dropZoneIdx = e.target.id.split('-')[1]
...
// column swap
      tdElement.ondragover = e =>  e.preventDefault()
      tdElement.ondrop = function (e) {
      const [rowdroppedElemIdx , coldroppedElemIdx ] =  e.dataTransfer.getData('text/plain').split('-')
      const [rowdropZoneIdx,coldropZoneIdx] = e.target.id.split('-')
      let swapValue = tableState.body[rowdroppedElemIdx].columnDetails[coldroppedElemIdx].cellValue

      tableState.body[rowdroppedElemIdx].columnDetails[coldroppedElemIdx].cellValue = tableState.body[rowdropZoneIdx].columnDetails[coldropZoneIdx].cellValue
      tableState.body[rowdropZoneIdx].columnDetails[coldropZoneIdx].cellValue= swapValue
      updateTableDataInLocalStorage(tableState)
      }
//
...
function updateTableBodyData(tableBodyData) {
  ...
  tableState.body.forEach((element) => {
    ...
    element.columnDetails.forEach((cellDetail) => {
      const tdElement = document.createElement(cellDetail.elementType);
      tdElement.setAttribute("draggale", true)
      const textAreaElement = getCellTextAreaElement(
        cellDetail.cellValue,
        `${cellDetail.rowNumber}-${cellDetail.columnNumber}`
      );
